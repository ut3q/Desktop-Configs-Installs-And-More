...
StyledWindow {
    ...
    Item {
        ...
        PanelBg {
            ...
            // Measured off the ClipWrapper because that is the item that
            // travels. The Wrapper inside it keeps the last popout's width
            // forever and retracts by sliding, which parked this blob's right
            // edge inside the bar and left a grey stub above the tray whenever
            // a popout had been opened once. That used to be hidden because the
            // bar was one continuous strip; it draws discrete section blocks
            // now, so the gaps between blocks show it.
            deformAmount: panels.popouts.isDetached ? 0.05 : panels.popouts.hasCurrent ? 0.15 : 0.1
            x: panels.popoutsWrapper.x + bar.implicitWidth - panels.popoutsWrapper.width * extraWidth
            implicitWidth: panels.popoutsWrapper.width * (1 + extraWidth)

            // The bar's section blocks round at sectionRadius (5) and only on
            // the two corners facing the desktop; PanelBg's default rounds all
            // four at 9. The popout was rounding precisely the corners the bar
            // refuses to, at nearly twice the radius, in a shell squared off
            // globally at rounding.scale 0.35.
            //
            // Its left edge sits past the bezel, so rounding there opens the
            // same wallpaper notch Bar.qml squares its blocks to avoid. Behind
            // a block that is invisible; in the gaps it is the difference
            // between a floating pill and something flush with the edge.
            //
            // Detached panels are centred with no bar to meet, so they keep it.
            radius: Tokens.rounding.large
            topLeftRadius: panels.popouts.isDetached ? radius : 0
            bottomLeftRadius: panels.popouts.isDetached ? radius : 0
