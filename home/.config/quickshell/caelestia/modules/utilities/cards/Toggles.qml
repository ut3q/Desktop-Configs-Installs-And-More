...
StyledRect {
    ...
    readonly property var quickToggles: {
        ...
        return Config.utilities.quickToggles.filter(item => {
            ...
            // Same reasoning as the bar's bluetooth icon: with no adapter the
            // toggle can only ever be off and clicking it does nothing, so drop
            // it from the grid rather than showing dead hardware. Comes back on
            // its own if an adapter appears.
            if (item.id === "bluetooth") {
                return Bluetooth.defaultAdapter !== null; // qmllint disable unresolved-type
            }
