...
StyledRect {
    ...
    implicitHeight: iconColumn.implicitHeight + Tokens.padding.small * 2 - (Config.bar.status.showLockStatus && !Hypr.capsLock && !Hypr.numLock ? iconColumn.spacing : 0)
    ...
    ColumnLayout {
        ...
        WrappedLoader {
            ...
            name: "bluetooth"
            // Gate on an adapter actually existing, not just on the config flag.
            // With no bluetooth hardware (nothing in /sys/class/bluetooth) the
            // icon was permanently stuck on "bluetooth_disabled" -- a control
            // for hardware that is not there. This still lights up the moment a
            // dongle is plugged in, which a config flag would not.
            // && short-circuits, so with showBluetooth off the Bluetooth
            // singleton is still never touched.
            active: Config.bar.status.showBluetooth && Bluetooth.defaultAdapter !== null // qmllint disable unresolved-type
