...
Singleton {
    ...
    Process {
        ...
        running: true
        // `nmcli monitor` outlives the shell whenever the shell dies by signal:
        // quickshell never gets to tear its children down, init adopts them, and
        // one more accumulates on every restart. Seven were found holding 61MB
        // and seven NetworkManager D-Bus connections between them. Let the
        // kernel do the teardown instead -- PR_SET_PDEATHSIG fires however the
        // parent goes, SIGKILL included, so the helper cannot be orphaned at
        // all. It survives the exec into nmcli, and the parent it watches is the
        // shell's main thread, which only ends when the process does.
