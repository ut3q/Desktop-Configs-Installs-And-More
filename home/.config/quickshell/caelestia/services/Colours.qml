...
Singleton {
    ...
    // ------------------------------------------------------------------
    // Surface darkening. Three tiers, deliberately ordered darkest first:
    //
    //   barSurface   the bar strip -- the darkest surface in the shell
    //   surfaceDarken  every other background, applied at scheme load
    //   barPill      the bar pills -- a step LIGHTER, so they read as raised
    //                objects sitting on the strip rather than holes in it
    //
    // Both bar tiers are based on `palette`, not `tPalette`: the transparent
    // palette runs container colours through alterColour(), which BRIGHTENS in
    // dark mode and pins alpha to transparency.layers. Working from the real
    // scheme colour keeps these two predictable relative to each other.
    //
    // On this scheme (catppuccin mocha) m3surface is already #131317, so the
    // strip's apparent lightness was never its hue -- it was the 0.5 alpha
    // letting the wallpaper through. Hence the opacity here: chasing the colour
    // darker alone does nothing over a bright wallpaper.
    readonly property real barSurfaceDarken: 1.3
    readonly property real barSurfaceOpacity: 0.9
    readonly property color barSurface: Qt.alpha(Qt.darker(palette.m3surface, barSurfaceDarken), barSurfaceOpacity)

    readonly property real barPillDarken: 1.0
    readonly property real barPillOpacity: 0.92
    readonly property color barPill: Qt.alpha(Qt.darker(palette.m3surfaceContainerHigh, barPillDarken), barPillOpacity)

    // Applied inside layer(), which is what produces every entry of tPalette --
    // so this one number reaches every background the shell paints: panels,
    // popouts, dashboard cards, the launcher, notifications, the lock screen.
    // Foreground colours are read from `palette` directly and are untouched, so
    // text and icon contrast only improves as this goes up.
    readonly property real surfaceDarken: 1.0
    ...
    readonly property var surfaceRe: /^m3(surface|background)/
    ...
    function load(data: string, isPreview: bool): void {
        ...
        for (const [name, colour] of Object.entries(scheme.colours)) {
            ...
            // Applied here rather than in layer() so it reaches the whole UI.
            // layer() only feeds tPalette, but plenty of backgrounds read
            // `palette` directly -- the nexus popups and ethernet rows, tray
            // and context menus, notification groups, the lock screen, the
            // recording modal. Those bypass layer() entirely and were the only
            // surfaces left at stock brightness.
            //
            // The regex matches the surface/background family only. Foreground
            // colours are named m3on* (m3onSurface, m3onSurfaceVariant,
            // m3onBackground), so they never match and text contrast only
            // improves as this goes up. m3inverseSurface is likewise excluded.
            colours[propName] = surfaceRe.test(propName) ? Qt.darker(`#${colour}`, root.surfaceDarken) : `#${colour}`;
        }
    }
