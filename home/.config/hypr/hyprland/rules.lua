...
-- Wine virtual desktops (Vinegar / Roblox Studio)
-- Wine is started with a FIXED desktop size (`/desktop=<uuid>,WxH`, from
-- Vinegar's studio.virtual_desktop). Left tiled, Hyprland handed the window
-- 1856x1064 once the bar reservation and gaps came off; the guest canvas never
-- fit its host window and Studio re-laid-out forever. So the window must be
-- floated at EXACTLY the Wine desktop size -- it tracks the guest resolution,
-- not the monitor.
--
-- 1872x1080 at 48,0 is the visible work area: the bar reserves 48px on the LEFT
-- (hyprctl prints that as reserved[0], despite the t/r/b/l label). Full-bleed
-- 1920x1080 at 0,0 also works, but the bar then covers Studio's left edge.
-- MUST stay in sync with studio.virtual_desktop in
...
hl.window_rule({ match = { class = "discord|equibop|vesktop|legcord|whatsapp" }, workspace = "special:communication" })
