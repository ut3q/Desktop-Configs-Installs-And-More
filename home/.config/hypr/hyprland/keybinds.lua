...
-- Kill/restart
--
-- These now cooperate with caelestia-shell-supervisor.sh, which restarts the
-- shell if it disappears unexpectedly. The stop file is how the supervisor
-- tells "the user meant this" from "something killed it", so kill-only writes
-- it and restart clears it. Both keep their original meaning:
--   CTRL + SUPER + SHIFT + R  kill and STAY down
--   CTRL + SUPER + ALT + R    restart, supervision stays armed
hl.bind(
	"CTRL + SUPER + SHIFT + R",
	hl.dsp.exec_cmd("mkdir -p ~/.local/state/caelestia-fork && touch ~/.local/state/caelestia-fork/supervisor.off; qs -c caelestia kill"),
	{ release = true }
)
hl.bind(
	"CTRL + SUPER + ALT + R",
	hl.dsp.exec_cmd("rm -f ~/.local/state/caelestia-fork/supervisor.off; qs -c caelestia kill; sleep .1; caelestia shell -d"),
	{ release = true }
)
...
hl.bind("SUPER + W", hl.dsp.exec_cmd("/home/aqua/Applications/Helium.AppImage"))
...
-- Select a region, OCR it, put the text (not the image) on the clipboard.
hl.bind("SUPER + SHIFT + T", hl.dsp.exec_cmd("~/.config/hypr/scripts/ocr-copy.sh"))
...
hl.bind("SHIFT + F8", hl.dsp.exec_cmd("~/.config/hypr/scripts/autoclicker.sh"))
hl.bind("CTRL + SHIFT + F8", hl.dsp.exec_cmd("~/.config/hypr/scripts/autokey.sh"))
hl.bind("SUPER + I", hl.dsp.exec_cmd("~/.config/hypr/scripts/devlayout.sh"))
--hl.bind("mouse:275", hl.dsp.exec_cmd("~/.config/hypr/scripts/minecraftauto.sh"))

-- Display rescue.
--
-- If the panel drops to "no signal" and stays there, the session behind it is
-- usually still alive -- what died is the DisplayPort link (the kernel logs
-- "enabling link 2 failed: 15" after dpcd_set_link_settings fails). This forces
-- a modeset at a refresh rate the link can definitely train, which recovers the
-- picture without a reboot. `locked = true` so it also works from the lock
-- screen, and both are chords you can hit blind.
--   CTRL + SUPER + ALT + D  -> force 1920x1080@60
--   CTRL + SUPER + ALT + F  -> put 1920x1080@170 back once you can see
hl.bind(
	"CTRL + SUPER + ALT + D",
	hl.dsp.exec_cmd("~/.config/hypr/scripts/display-rescue.sh rescue"),
	{ locked = true }
)
hl.bind(
	"CTRL + SUPER + ALT + F",
	hl.dsp.exec_cmd("~/.config/hypr/scripts/display-rescue.sh restore"),
	{ locked = true }
)
